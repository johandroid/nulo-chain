...
//!
//! Enables cross-chain token transfers via HyperFungibleToken and
//! WrappedHyperFungibleToken Solidity contracts.
...
pub mod pallet {
    ...
    /// Maps (StateMachine, AssetId) to the token contract address on that chain.
    #[pallet::storage]
    ...
    /// Reverse lookup: (StateMachine, contract address bytes) to local asset ID.
    #[pallet::storage]
